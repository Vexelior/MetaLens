...
/**
 * Build an Error carrying everything the API needs to give the user a clear,
 * actionable message: an HTTP status, a machine-readable code, a short
 * human sentence (`message`), and a longer `detail` explaining the likely
 * cause and what to do about it.
 */
function metadataError(message, statusCode, code, detail) {
...
async function extractMetadata(buffer, filename = 'unknown') {
  if (!Buffer.isBuffer(buffer) || buffer.length === 0) {
    throw metadataError(
      ...
      400,
      'EMPTY_FILE',
      'It contains 0 bytes, so there is nothing to read. This usually means the ' +
        'file did not finish downloading or saving. Try uploading a fresh copy of the image.'
    );
  }

  if (buffer.length > MAX_FILE_BYTES) {
    throw metadataError(
      `File too large. Maximum size is ${humanFileSize(MAX_FILE_BYTES)}.`,
      413,
      'FILE_TOO_LARGE',
      `Your file is ${humanFileSize(buffer.length)}, which is over the ` +
        `${humanFileSize(MAX_FILE_BYTES)} limit. Resize or compress the image and try again.`
    );
  }
  ...
  if (!format) {
    throw metadataError(
      ...
      415,
      'UNSUPPORTED_FORMAT',
      "MetaLens identifies images by their actual contents, not their file name. This file's " +
        'bytes do not match any supported image format. It may be a non-image file (such as a ' +
        'PDF, ZIP, or video), or a file that was renamed to an image extension. Try uploading a ' +
        'genuine JPEG, PNG, GIF, WebP, BMP, TIFF, AVIF or ICO image.'
    );
  }
